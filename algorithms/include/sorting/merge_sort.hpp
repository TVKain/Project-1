    ...
    namespace sort {
        ...
        void merge_sort_iterative(T *first, T *last) {
            ...
            for (; group_size < size; group_size *= 2) {
                ...
                while (start + 2 * group_size < last) {
                    auto end = start + 2 * group_size;
                    auto mid = start + group_size;

                    merge(start, mid, end, [](const T  &a, const T &b) {
                        return a < b;
                    });
            ...
            merge(first, first + group_size / 2, last);
        }    
